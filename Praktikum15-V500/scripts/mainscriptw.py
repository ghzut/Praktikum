...
# Werte-Tabellen
makeTable([gelb[0][0:len(gelb[0])//2]*10**3, gelb[1][0:len(gelb[1])//2]*10**12], r'{ $U/\si{\milli\volt}$ } & { $I/\si{\nano\ampere}$ }' ,'tabgelb1' , ['S[table-format=5.0]', 'S[table-format=4.0]'] ,  ["%5.0f", "%4.0f"])
makeTable([gelb[0][len(gelb[0])//2:]*10**3, gelb[1][len(gelb[1])//2:]*10**12], r'{ $U/\si{\milli\volt}$ } & { $I/\si{\nano\ampere}$ }' ,'tabgelb2' , ['S[table-format=5.0]', 'S[table-format=4.0]'] ,  ["%5.0f", "%4.0f"])

makeTable([gruen[0]*10**3, gruen[1]*10**12], r'{ $U/\si{\milli\volt}$ } & { $I/\si{\nano\ampere}$ }' ,'tabgruen' , ['S[table-format=3.0]', 'S[table-format=3.0]'] ,  ["%3.0f", "%3.0f"])

makeTable([blaugruen[0]*10**3, blaugruen[1]*10**12], r'{ $U/\si{\milli\volt}$ } & { $I/\si{\nano\ampere}$ }' ,'tabblaugruen' , ['S[table-format=3.0]', 'S[table-format=1.0]'] ,  ["%3.0f", "%1.0f"])

makeTable([violett[0]*10**3, violett[1]*10**12], r'{ $U/\si{\milli\volt}$ } & { $I/\si{\nano\ampere}$ }' ,'tabviolett' , ['S[table-format=3.0]', 'S[table-format=3.0]'] ,  ["%3.0f", "%3.0f"])

makeTable([ultraV1[0]*10**3, ultraV1[1]*10**12], r'{ $U/\si{\milli\volt}$ } & { $I/\si{\nano\ampere}$ }' ,'tabultraV1' , ['S[table-format=4.0]', 'S[table-format=2.0]'] ,  ["%4.0f", "%2.0f"])

makeTable([ultraV2[0]*10**3, ultraV2[1]*10**12], r'{ $U/\si{\milli\volt}$ } & { $I/\si{\nano\ampere}$ }' ,'tabultraV2' , ['S[table-format=4.0]', 'S[table-format=2.0]'] ,  ["%4.0f", "%2.0f"])

# Zwischenergebnis-Tabelle(n)
makeTable([Wellenlaengen*10**9, Frequenzen*10**(-12), nom(Nullstellen), std(Nullstellen)], r'{ $\lambda/\si{\nano\meter}$ } & { $f/\si{\per\pico\second}$ } & \multicolumn{2}{c}{ $U_\text{g}/\si{\volt}$ }' ,'tabZwischenErgebnisse' , ['S[table-format=3.1]', 'S[table-format=3.2]', 'S[table-format=1.2]', '@{${}\pm{}$}S[table-format=1.2]'] ,  ["%3.1f", "%3.2f", "%1.2f", "%1.2f"])
