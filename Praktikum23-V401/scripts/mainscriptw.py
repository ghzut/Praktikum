...
makeNewTable(data=a, names=r'{$\frac{\Delta s}{\si{\milli\meter}}$} {$N$}', filename='a', formats=[r'S[table-format=1.2]',r'S[table-format=4.0]'], formats2=[r'{:3.2f}',r'{:4.0f}'])
...
makeNewTable(data=b, names=r'{$\frac{\Delta p}{\si{\bar}}$} {$N$}', filename='b', formats=[r'S[table-format=1.2]',r'S[table-format=2.0]'], formats2=[r'{:3.2f}',r'{:2.0f}'])
...
def brechungsindex(deltan,deltap,T):
    return 1 + deltan*(T/const.zero_Celsius)*(const.atmosphere)/deltap

def deltan(N, lambdaLaser):
    return (N/2)*(lambdaLaser/L)

deln = deltan(N, Lambda)
n = brechungsindex(deln,p,293.15)
print(n)
