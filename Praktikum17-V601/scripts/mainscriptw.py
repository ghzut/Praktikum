...
plt.cla()
plt.clf()
plt.plot(unp.nominal_values(a1Grad[0]), np.tan(a1Grad[1]/360*2*np.pi), 'rx', label='Messwertepaare')
...
plt.cla()
plt.clf()
plt.plot(unp.nominal_values(a2Grad[0]), np.tan(a2Grad[1]/360*2*np.pi), 'rx', label='Messwertepaare')
#plt.ylim(0, line(x[-1], *unp.nominal_values(a2params))+0.1)
#plt.xlim(0, x[-1])
plt.xlabel(r'$U_\text{A}/\si{\volt}$')
plt.ylabel(r'$\overline{I}$')
